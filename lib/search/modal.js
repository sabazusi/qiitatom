...
import {LOCAL_STORAGE_KEYNAME} from '../constants';
...
export default class extends View {
  ...
  initialize() {
    ...
    atom.workspace.addOpener((uri) => {
      ...
      if (protocol === 'qiitatomsearch:') return new SearchResult(host);
    });
  ...
  onClickAuthButton() {
    if (this.hasAuthenticated) {
      if (confirm('現在の認証情報を削除しますか?')) {
        ...
        localStorage.removeItem(LOCAL_STORAGE_KEYNAME);
      ...
      if (!this.authModal) {
        const onCompleted = (token) => {
          ...
          localStorage.setItem(LOCAL_STORAGE_KEYNAME, token);
        };
        const onCanceled = () => {
          this.toggleAuthPanel(false);
        };
  ...
  auth() {
    const authToken = localStorage.getItem(LOCAL_STORAGE_KEYNAME, '');
