...
export default class extends View {
  static content() {
    this.div({class: 'qiitatom-post-modal'}, () => {
      this.div({class: 'qiitatom-post-modal__file'}, () => {
        this.label('投稿対象ファイル:');
        this.label({outlet: 'targetFileName'}, '');
      });
      this.div({class: 'qiitatom-post-modal__titleinput'}, () => {
        this.label({class: 'qiitatom-post-modal__titleinput-label'}, 'タイトル');
        this.subview('postTitleInput', new TextEditorView({mini: true, placeholderText: '投稿タイトル'}));
      });
      this.div({class: 'qiitatom-post-modal__taginput'}, () => {
      ...
      this.div({class: 'qiitatom-post-modal__publicity'}, () => {
        this.input({
          outlet: 'public',
          class : 'qiitatom-post__publicity-button',
          id    : 'public',
          type  : 'radio',
          value : 'title',
          name  : 'visibilityType'
        ...
        this.label({
          for  : 'public',
          class: 'qiitatom-post__publicity-label'
        }, '公開');
        this.input({
          outlet : 'private',
          class  : 'qiitatom-post__publicity-button',
      ...
      this.div({class: 'qiitatom-post-modal__buttons'}, () => {
        this.label({
          outlet: 'authButtonLabel',
          class : 'qiitatom-post-modal__buttons--authButtonLabel'
        }, '投稿にはqiitaアカウントの認証が必要です');
        this.button({
          outlet: 'authButton',
          class : 'qiitatom-post__modal_buttons--authButton'
        }, '認証する');
        this.button({
          outlet: 'previewButton',
          class : 'qiitatom-post-modal__buttons--previewButton'
        }, 'プレビューを表示する');
      });
    });
  }
    ...
    atom.workspace.open(`${previewProtocol}//preview`, {split: 'right'})
      .then((view) => {
        view.render(
          ...
          tags.filter((tag, index, array) => array.indexOf(tag) === index),
