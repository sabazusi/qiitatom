...
export default class extends View {
  static content() {
    this.div({class: 'qiitatom'}, () => {
      ...
      this.button({outlet: 'authButton', class: 'qiitatomAuthButton'}, '認証する');
  ...
  initialize() {
    ...
    getClient('authToken').auth().then((authenticated) => {
  ...
  post() {
    ...
    let api;
    if (this.typeTitle.is(':checked')) api = client.searchByTitle;
    if (this.typeTag.is(':checked')) api = client.searchByTag;
    if (this.typeKeyword.is(':checked')) api = client.searchByKeyword;
    if (!api) return;
